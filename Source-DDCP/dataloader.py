...
'''
def resize_image(origin_img, long_edge=1024, short_edge=960): # padding the image to 1024x960
	# long_edge, short_edge = 2048, 1920
	# long_edge, short_edge = 1024, 960
	# long_edge, short_edge = 512, 480


	im_lr = origin_img.shape[0] # height of the image
	im_ud = origin_img.shape[1] # width of the image
	new_img = np.zeros([long_edge, short_edge, 3], dtype=np.uint8) # create a new image with 1024x960
	new_shape = new_img.shape[:2] # get the shape of the new image
	if im_lr > im_ud: # 如果高大于宽，那么就以高为基准，等比例缩放，然后填充宽
		img_shrink, base_img_shrink = long_edge, long_edge # the height of the new image
		im_ud = int(im_ud / im_lr * base_img_shrink) # calculate the new width
		im_ud += 32-im_ud%32 # make sure the width is a multiple of 32
		im_ud = min(im_ud, short_edge) # if the width is larger than 960, then set it to 960
		im_lr = img_shrink # the height of the new image
	...
	else: # 如果宽大于高，那么就以宽为基准，等比例缩放，然后填充高
